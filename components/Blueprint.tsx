...
const Blueprint: React.FC<BlueprintProps> = ({ 
  ...
  const gridSnapPx = PIXELS_PER_FOOT * GRID_SNAP_FEET;
  const gridBackgroundImage = viewMode === '2d' ? `
    linear-gradient(${gridColor} 1px, transparent 1px), 
    linear-gradient(90deg, ${gridColor} 1px, transparent 1px)
  ` : 'none';
  const gridBackgroundSize = viewMode === '2d' ? `${gridSnapPx}px ${gridSnapPx}px` : 'auto';
...
const RoomComponent: React.FC<RoomComponentProps> = ({ room, isSelected, onSelect, onUpdateRoom, onFinalizeUpdate, viewMode, isDarkMode }) => {
    ...
    const renderFeature = (feature: Feature) => {
        ...
        switch (feature.type) {
            ...
            case FeatureType.FrenchDoor:
                featureClass = 'bg-transparent';
                 children = (
                    <>
                        <div className="h-full bg-[#a0522d]" style={{width: '45%'}}></div>
                        <div className="h-full bg-[#a0522d]" style={{width: '45%'}}></div>
                    </>
                );
                break;
        ...
        if (feature.type === FeatureType.Outlet) {
          const iconStyle: React.CSSProperties = {position: 'absolute'};
          if (feature.wall === 'top') { iconStyle.top = '-12px'; iconStyle.left = `${offsetPx * 100}%`; iconStyle.transform = 'translateX(-50%)'; }
          if (feature.wall === 'bottom') { iconStyle.bottom = '-12px'; iconStyle.left = `${offsetPx * 100}%`; iconStyle.transform = 'translateX(-50%)'; }
          if (feature.wall === 'left') { iconStyle.left = '-12px'; iconStyle.top = `${offsetPx * 100}%`; iconStyle.transform = 'translateY(-50%)'; }
          if (feature.wall === 'right') { iconStyle.right = '-12px'; iconStyle.top = `${offsetPx * 100}%`; iconStyle.transform = 'translateY(-50%)'; }
          return <div key={feature.id} style={iconStyle} className="text-slate-600 dark:text-slate-400 z-10"><OutletIcon size={12}/></div>
        }
        
        return <div key={feature.id} style={style} className={`z-10 rounded-sm ${featureClass}`}>{children}</div>
    }
    ...
    return (
        <div
            ...
            style={{
                ...
                transition: 'left 0s, top 0s, all 0.2s ease-out', // No transition for position during drag
                transformStyle: 'preserve-3d',
                zIndex: 10
            }}
        ...
        >
            ...
            >
                 {room.type === RoomType.Stairs && viewMode === '2d' && (
                    <div className="absolute inset-0 overflow-hidden pointer-events-none">
                        <div style={{
                            ...
                            backgroundImage: `repeating-linear-gradient(135deg, rgba(0,0,0,0.05), rgba(0,0,0,0.05) 1px, transparent 1px, transparent 10px)`
                        }} className="dark:bg-[repeating-linear-gradient(135deg,rgba(255,255,255,0.05),rgba(255,255,255,0.05)_1px,transparent_1px,transparent_10px)]" />
                    </div>
                )}
                {viewMode === '3d' && (
                    <>
                        <div className="absolute inset-0 bg-slate-300 dark:bg-slate-800" style={{ transform: `translateZ(-${WALL_HEIGHT_PIXELS}px)` }}></div>
                        
                        {room.type === RoomType.Stairs ? (
                             <div className="absolute inset-0 bg-slate-400 dark:bg-slate-600" style={{
                                transform: 'perspective(200px) rotateX(45deg)',
                                backgroundImage: `repeating-linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.1) 1px, transparent 1px, transparent 15px)`,
                                transformOrigin: 'bottom',
                             }}></div>
                        ) : (
                            <>
                                {!hasOpening('top') && <div className="wall" style={{...wall3DStyle, opacity: isFrontFacing('top') ? 0 : 1, width: roomWidthPx, height: WALL_HEIGHT_PIXELS, transform: `rotateX(90deg)`, transformOrigin: 'top' }}></div>}
                                {!hasOpening('bottom') && <div className="wall" style={{...wall3DStyle, opacity: isFrontFacing('bottom') ? 0 : 1, width: roomWidthPx, height: WALL_HEIGHT_PIXELS, transform: `rotateX(-90deg)`, transformOrigin: 'bottom' }}></div>}
                                {!hasOpening('left') && <div className="wall" style={{...wall3DStyle, opacity: isFrontFacing('left') ? 0 : 1, width: roomLengthPx, height: WALL_HEIGHT_PIXELS, transform: `rotateY(-90deg)`, transformOrigin: 'left' }}></div>}
                                {!hasOpening('right') && <div className="wall" style={{...wall3DStyle, opacity: isFrontFacing('right') ? 0 : 1, width: roomLengthPx, height: WALL_HEIGHT_PIXELS, transform: `rotateY(90deg)`, transformOrigin: 'right' }}></div>}
                            </>
                        )}
                    </>
                )}
                ...
                <div style={textRotationStyle}>
                    <div className="p-2 text-center select-none" style={{transform: `translateZ(1px)`}}>
...
const FurnitureComponent: React.FC<FurnitureComponentProps> = ({ furniture, viewMode, onFinalizeUpdate, room }) => {
    ...
    return (
        ...
        >
            <div 
                className="absolute inset-0 border-2 border-black/20 rounded-sm flex items-center justify-center"
                style={{ backgroundColor: furnitureColor }}
            >
                 <div className="w-full h-full flex items-center justify-center" style={{transform: `rotate(-${furniture.rotation}deg)`}}>
                    <p className="text-white text-xs font-semibold select-none p-1 text-center" style={{ writingMode: furnitureWidthPx < 40 ? 'vertical-rl' : 'horizontal-tb', textShadow: '1px 1px 1px rgba(0,0,0,0.5)' }}>
             ...
             {viewMode === '3d' && (
                 <>
                    <div className="absolute inset-0 rounded-sm" style={{backgroundColor: furnitureDarkerColor, transform: `translateZ(-${furnitureHeightPx}px)`}}></div>
                    <div className="wall-3d" style={{ backgroundColor: `${furnitureDarkerColor}B3`, width: furnitureWidthPx, height: furnitureHeightPx, transform: 'rotateX(90deg)', transformOrigin: 'top'}}></div>
                    <div className="wall-3d" style={{ backgroundColor: `${furnitureDarkerColor}B3`, width: furnitureWidthPx, height: furnitureHeightPx, transform: 'rotateX(-90deg)', transformOrigin: 'bottom'}}></div>
                    <div className="wall-3d" style={{ backgroundColor: `${furnitureDarkerColor}B3`, width: furnitureLengthPx, height: furnitureHeightPx, transform: 'rotateY(-90deg)', transformOrigin: 'left'}}></div>
                    <div className="wall-3d" style={{ backgroundColor: `${furnitureDarkerColor}B3`, width: furnitureLengthPx, height: furnitureHeightPx, transform: 'rotateY(90deg)', transformOrigin: 'right'}}></div>
                 </>
             )}
        </div>
    );
...
const PetComponent: React.FC<PetComponentProps> = ({ pet, viewMode, onFinalizeUpdate, room }) => {
    ...
    return (
        <div
            ...
            title={pet.name}
        >
            <div style={{ textShadow: '0px 0px 5px rgba(0,0,0,0.5)' }}>
